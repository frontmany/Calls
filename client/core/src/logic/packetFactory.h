...
namespace core {
    ...
    public:
        ...
        static std::vector<unsigned char> getAuthorizationPacket(const std::string& myNickname, const CryptoPP::RSA::PublicKey& myPublicKey, uint16_t udpPort);
        static std::vector<unsigned char> getReconnectPacket(const std::string& myNickname, const std::string& myToken, uint16_t udpPort);
        static std::vector<unsigned char> getRequestUserInfoPacket(const std::string& myNickname, const std::string& userNickname);
