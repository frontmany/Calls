    ...
    const unsigned char* payload, std::size_t payloadSize)
{
...

    {
        ...
        if (!m_pendingPacket.has_value() || m_pendingPacket->totalChunks != chunkCount) {
            m_pendingPacket = PendingPacket{};
            m_pendingPacket->totalChunks = chunkCount;
            m_pendingPacket->chunks.resize(chunkCount);
            m_pendingPacket->receivedChunks = 0;
            m_pendingPacket->type = packetType;
            m_pendingPacket->typeSet = true;
        }
        ...
        auto& packet = m_pendingPacket.value();
