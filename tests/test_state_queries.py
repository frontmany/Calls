...
def state_query_scenario(client, handler):
    ...
    assert client.get_my_nickname() == handler.name, f"Nickname should be {handler.name}"
...
class StateQueriesTest(TestRunner):
    def test_state_queries(self):
        ...
        process = multiprocessing.Process(
            ...
            args=("localhost", self.port, "state_test_user", events, "state_test_user", state_query_scenario, 3)
        )
