...
def screen_sharing_caller_scenario(client, handler):
    ...
    if handler.wait_for_event("calling_accepted", 5):
        ...
        dummy_data = bytes([0x01, 0x02, 0x03, 0x04] * 100)
...
def screen_sharing_responder_scenario(client, handler):
...
class ScreenSharingTest(TestRunner):
    def test_screen_sharing(self):
        ...
        responder_process = multiprocessing.Process(
            ...
            args=("localhost", self.port, "responder", responder_events, "responder", screen_sharing_responder_scenario, 10)
        )
        
        caller_process = multiprocessing.Process(
            target=run_client_flexible,
            args=("localhost", self.port, "caller", caller_events, "caller", screen_sharing_caller_scenario, 10)
        )
