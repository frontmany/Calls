...
void MainWindow::executePrerequisites() {
    ...
    QTimer::singleShot(0, [this]()
    {
        ...
        if (!firstInstance && !multipleInstancesAllowed) {
            // DialogsController будет создан в init()
            // Пока просто не показываем диалог
        } else {
            // UpdateManager будет создан в init()
            // Проверка обновлений будет запущена после инициализации
        }
    });
}
...
void MainWindow::init() {
    ...
    m_audioManager = new AudioEffectsManager(this);
    m_audioSettingsManager = new AudioSettingsManager(m_configManager, this);
    m_updateManager = new UpdateManager(m_configManager, this);
    m_authorizationManager = nullptr; // Created after navigation controller
    m_callManager = new CallManager(m_audioManager, nullptr, this); // Navigation controller set later
    m_screenSharingManager = new ScreenSharingManager(this);
    m_cameraSharingManager = new CameraSharingManager(m_configManager, this);
...
void MainWindow::setupUI() {
    ...
    m_networkErrorHandler = new NetworkErrorHandler(m_navigationController, m_updateManager, m_configManager, this);
